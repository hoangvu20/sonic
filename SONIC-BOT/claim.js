  ...
  try {
    ...
    for (let i = 0; i < PRIVATE_KEYS.length; i++) {
      ...
      while (availableBoxes > 0) {
        console.log(`[ ${moment().format('HH:mm:ss')} ] Opening box ${profile.ring_monitor - availableBoxes + 1} of ${profile.ring_monitor}`.yellow);
        ...
        if (openedBox.data.success) {
          ...
          console.log(`[ ${moment().format('HH:mm:ss')} ] Failed to open box. Retrying...`.red);
        }
      ...
      console.log(`[ ${moment().format('HH:mm:ss')} ] Performing daily check-in...`.yellow);
      const checkinResponse = await dailyLogin(token, keypair);

      if (checkinResponse && checkinResponse.data.success) {
        console.log(
          `[ ${moment().format('HH:mm:ss')} ] Daily check-in successful! Status: ${checkinResponse.status} | Accumulative Days: ${checkinResponse.data.accumulative_days}`.green
        );
      } else {
        console.log(`[ ${moment().format('HH:mm:ss')} ] Daily check-in failed or already completed.`.red);
      }

      console.log(`Daily check-in completed for private key ${i + 1}.`.cyan);
    }

    console.log('All private keys processed, all boxes opened, and daily check-ins completed.'.cyan);
  } catch (error) {
